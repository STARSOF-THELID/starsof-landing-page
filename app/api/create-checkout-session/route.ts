  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json(
        {
          error:
            "Error checking user existing subscription. Please contact StarsOf AR team.",
        },
        { status: 500 },
      );
    }
    ...
    const url = `${STARSOF_SERVER_URL}/payment${TEST_MODE_ENABLED ? "/test" : ""}/create-checkout-session`;
